 ...
 * should NOT use the methods in this class - use the methods in the Info
 * classes instead.
 */
public class database
{
	...
	{
		this(name, user, password, "jdbc:mysql://localhost:3306",
				"com.mysql.jdbc.Driver");
	}

	public database(String name, String user, String password,
			String connection_url, String driver_name)
	{
		this.name = name;
		this.user = user;
	...
	public int record_count(String target, String source, String field,
			Object value)
	{
		return record_count(target, source, field + " = " + value_string(value));
	}

	public int record_count(String target, String source, String field1,
			Object value1, String field2, Object value2)
	{
		return record_count(target, source, field1 + " = "
				+ value_string(value1) + " And " + field2 + " = "
				+ value_string(value2));
	}

	public void select(String target, String source, String criteria,
			String order)
	{
		try
		{
			...
			if (criteria.isEmpty())
				...
				s.execute("Select " + target + " From " + source + " Where "
						+ criteria + ordering);
			if (results != null)
				results.close();
			results = s.getResultSet();
	...
	public Boolean select_record(String target, String source, String field,
			Object value)
	{
	...
	public Boolean select_record(String source, String field1, Object value1,
			String field2, Object value2)
	{
		select("*", source, field1 + " = " + value_string(value1) + " And "
				+ field2 + " = " + value_string(value2), "");
		return move_first();
	}
	...
	public int find_id(String table, String field, Object value)
	{
		...
		if (move_first())
			return (Integer) get_field(table + "_id");
		else
			return 0;
	}

	public int find_id(String id_field, String source, String field1,
			Object value1, String field2, Object value2)
	{
		select(id_field, source, field1 + " = " + value_string(value1)
				+ " And " + field2 + " = " + value_string(value2), "");
		if (move_first())
			return (Integer) get_field(id_field);
		else
			return 0;
	}
	...
	public int[] select_ids(String id_field, String source, String field,
			Object value, String order)
	{
		int count = record_count(id_field, source, field + " = "
				+ value_string(value));
		int[] results = new int[count];
	...
	public int[] select_ids(String id_field, String source, String field1,
			Object value1, String field2, Object value2, String order)
	{
		int count = record_count(id_field, source, field1 + " = "
				+ value_string(value1) + " And " + field2 + " = "
				+ value_string(value2));
		int[] results = new int[count];
		select(id_field, source, field1 + " = " + value_string(value1)
				+ " And " + field2 + " = " + value_string(value2), order);
		for (int i = 0; i < count && move_next(); i = i + 1)
			results[i] = (Integer) get_field(id_field.replaceFirst("Distinct ",
					""));
		return results;
	...
	public static String join(String referring_table, String referenced_table,
			String referring_field)
	{
		return referring_table + " Inner Join " + referenced_table + " On ("
				+ referenced_table + "." + referenced_table + "_id = "
				+ referring_table + "." + referring_field + ")";
	}
	...
	public Boolean begin_update(String table, Integer id)
	{
		try
		{
			...
			s.execute("Select * From " + table + " Where " + table + "_id = "
					+ id.toString());
			if (results != null)
				results.close();
			results = s.getResultSet();
	...
	{
		return execute("Delete From " + table + " Where " + table + "_id = "
				+ id.toString());
	}
	...
	{
		return execute("Delete From " + table + " Where field = "
				+ value_string(value));
	}

	public Boolean delete_record(String table, String field1, Object value1,
			String field2, Object value2)
	{
		return execute("Delete From " + table + " Where " + field1 + " = "
				+ value_string(value1) + " And " + field2 + " = "
				+ value_string(value2));
	}
	...
	public String value_string(Object x)
	{
		if (x == null)
			...
			return sql_date((java.util.Date) x);
		else
			return x.toString();
	}
	...
	private Boolean find_driver()
	{
		try
		{
			...
			if (driver_class == null)
				driver_class = Class.forName(driver_name, true,
